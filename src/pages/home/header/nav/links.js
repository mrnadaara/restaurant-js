...
export default class Links {
  constructor() {
    this.navList = ['Home', 'Location', 'About'];
  }
  ...
  renderButton() {
    ...
    const span = DOMHelper.createElement('span', ['navbar-toggler-icon']);
    button.appendChild(span);
    return button;
  }

  renderNavItem(navItem) {
    const navItemElement = DOMHelper.createElement('li', ['nav-item']);
  ...
  renderNavList() {
    ...
    const navListElement = DOMHelper.createElement('ul', [
      'navbar-nav',
      'd-flex',
  ...
  renderNavElement() {
    const navElement = DOMHelper.createElement('nav', [
      'navbar',
      'navbar-expand-lg',
      'navbar-dark',
    ]);
  ...
  render() {
    const linkContainer = DOMHelper.createElement('div', ['col-3']);
